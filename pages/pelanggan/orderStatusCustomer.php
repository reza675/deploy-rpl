...
SELECT 
    ...
    b.gambarBeras,
    b.beratBeras,
    po.namaPemilik AS businessOwner,
    po.alamatPemilik AS alamatBusiness,
    po.nomorHPPemilik AS businessContact,
...
JOIN pemilikusaha po ON pg.idPemilik = po.idPemilik
WHERE pg.idPelanggan = '$idPelanggan'
...
<head>
    ...
    <title>Order Status - Customer View</title>
...
<body class="bg-[#EFE9E2] min-h-screen">
    ...
    <div class="main-container ml-[300px] mt-4 mr-12">
        <div class="flex justify-between items-center gap-6">
            <div class="flex-shrink-0">
                ...
                <p class="text-l text-[#5D5C61] font-regular">Track your orders</p>
            </div>
        ...
        <!-- Tampilan Utama -->
        <div class="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8 mb-4">
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-xl font-bold text-[#16151C] mb-4">Your Orders</h3>
                <div class="space-y-4">
                    ...
                    <div class="p-4 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer order-item
                        ...
                        data-business="<?= htmlspecialchars($pesanan['businessOwner']) ?>"
                        data-alamat="<?= htmlspecialchars($pesanan['alamatBusiness']) ?>"
                        data-contact="<?= htmlspecialchars($pesanan['businessContact']) ?>"
            ...
            <div class="lg:col-span-2 bg-white rounded-lg shadow p-6">
                ...
                <div class="mt-8 border-t pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <p class="font-semibold text-gray-700">Seller Information</p>
                            ...
                            <div class="customer-info">
                                ...
                                <div>
                                    <p class="font-semibold text-[#16151C] mt-1" id="detail-business">-</p>
                    ...
                    <!-- Status Information -->
                    <div class="status-info mt-6">
                        <h4 class="font-bold text-[#16151C] mb-2">Status Information</h4>
                        <p id="status-info-text">Select an order to view status details</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Informasi status untuk pelanggan
        const statusInfo = {
            'order placed': 'Your order has been received and is being processed by the seller.',
            'packaging': 'The seller is preparing your order for shipment.',
            'on the road': 'Your order is on its way to you. Estimated delivery time is 1-2 business days.',
        ...
        function updateOrderDetail(data) {
            ...
            document.getElementById('detail-business').textContent = data.business;
            ...
            const statusText = statusInfo[data.status] || 'Status information not available';
            document.getElementById('status-info-text').textContent = statusText;

            const orderPlacedTime = document.querySelector(
                '.status-step[data-step="order placed"] .status-time');
            if (orderPlacedTime) {
                orderPlacedTime.textContent = data.tanggal || 'Time not available';
            }
            ...
            // Update waktu untuk status yang sudah dilalui
            if (currentStatus !== 'order placed') {
                document.querySelector('.status-step[data-step="order placed"] .status-time').textContent = data
                    .tanggal || 'Time not available';
            }
        }
