...
if (!empty($_GET['search'])) {
    ...
    $whereClauses[] = "(p.namaPelanggan LIKE '%{$s}%' OR sb.namaBeras LIKE '%{$s}%')";
}
...
// fetch ke array
$dataPesananPelanggan = [];
while ($row = mysqli_fetch_assoc($query)) {
    $dataPesananPelanggan[] = $row;
}
        ...
        <div class="rounded-2xl border border-[#A2A1A8] shadow-lg p-8 mt-4 mb-4">
            <div class="relative flex justify-between items-center gap-4">
                <div class="relative flex-grow">
                    <form action="historyOrder.php" method="get" class="relative">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                            class="absolute w-5 h-5 top-2.5 left-2.5 text-slate-600">
                            <path fill-rule="evenodd"
                                d="M10.5 3.75a6.75 6.75 0 1 0 0 13.5 6.75 6.75 0 0 0 0-13.5ZM2.25 10.5a8.25 8.25 0 1 1 14.59 5.28l4.69 4.69a.75.75 0 1 1-1.06 1.06l-4.69-4.69A8.25 8.25 0 0 1 2.25 10.5Z"
            ...
            <div class="overflow-x-auto mt-4">
                <table class="min-w-full bg-white rounded-lg shadow">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <?php
                            if (empty($dataPesananPelanggan)):
                        ?>
                        <tr>
                            <td colspan="9" class="px-4 py-3 text-center text-gray-500">No data found</td>
                        </tr>
                        <?php else: ?>
                        <?php foreach ($dataPesananPelanggan as $index => $row): ?>
