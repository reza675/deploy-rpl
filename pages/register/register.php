...
<body class="bg-[#EFE9E2] min-h-screen flex">
    ...
    <?php
        ...
        if (isset($_GET['register']) && $_GET['register'] == "gagal_daftar"){
            echo "<div id='failedPopup' class='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden'>
                <div class='bg-white rounded-2xl p-8 max-w-md text-center shadow-xl'>
                    <div class='mb-4'><img src='../../assets/gambar/register/failed.png' alt='failed' class='w-24 h-24 mx-auto mix-blend-multiply'></div>
                    <h3 class='text-xl font-bold mb-2'>Account Create Failed</h3>
                    <p class='text-gray-600 mb-6'>Name already used. Try again.</p>
                    <a href='register.php'
                        class='inline-block px-6 py-2 bg-[#8b5c2c] hover:bg-[#6f451e] text-white rounded-lg font-medium'>
                        Back to Register
                    </a>
                ...
                <form action="../../assets/mysql/register_login/proses.php" method="POST" class="space-y-4">
                    <div>
                        ...
                        <input type="email" name="email" id="email" placeholder="Email Address"
                            class="w-full bg-[#fef9f4] border border-[#c0a080] rounded-lg p-3 text-gray-800" required>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="password" name="password" id="password" placeholder="Password"
                                class="w-full bg-[#fef9f4] border border-[#c0a080] rounded-lg p-3 pr-10 text-gray-800"
                                required>
    ...
    window.onload = function() {
        ...
        if (urlParams.get('register') === 'berhasil') {
            document.getElementById("successPopup").classList.remove("hidden");
        }
        else if (urlParams.get('register') === 'gagal_daftar') {
            document.getElementById("failedPopup").classList.remove("hidden");
        }
